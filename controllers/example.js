 ...
 */
function _isLinkOk(link, visited, path) {
  /**
 ...
 */
function _goToNext(url, visited, path, callbackEach, callback) {
  ...
  request(url, function (error, response, body) {
    ...
    if (!!content.html() && !noArticle.html()) {
      ...
      var i = 1;
      while (!_isLinkOk(link, visited, path)) {
        link = $(selector).eq(i);
        url = link.attr('href');
        i = i + 1;
      }
      ...
      _goToNext('http://en.wikipedia.com/' + url, visited, path, callbackEach, callback);
...
exports.start = function (page, callbackEach, callback) {
  ...
  var visited = [];
  var path = [];
  _goToNext('http://en.wikipedia.com/wiki/' + page, visited, path, callbackEach, callback);
};